 ...
 *
 * 42 个 channel 的 handler 逻辑本就宿主中立，Electron 耦合仅在传输：
 ...
 *   · Electron：host-electron/ipc.ts 的 electronIpcSurface()（token＝
...
// 持有宿主对象，经这里取宿主在装配段（Electron main.ts / sidecar server.ts）
...
/** handler 收到的最小事件上下文（无 Electron IpcMainEvent 依赖）。 */
 ...
 * 单向推送）。Electron 映射 ipcMain.handle/on；sidecar 映射 JSON-RPC
 * 方法注册（Task 7.1）。
 */
 ...
 * Electron 端语义（含未登记 channel 的调用报错）。
 */
