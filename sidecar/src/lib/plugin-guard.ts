...
// 忠实移植自仓库根 plugin-guard.js（设计背景见原文件头注释）：
// 快照/回滚、静态体检、修复执行器、junction 归属守卫、事故报告、守护启动、
// 启动失败归因。只动插件层与配置层，绝不修改 harness 内核或用户会话数据。
...
const TROJAN_PATTERNS: { code: string; re: RegExp }[] = [
...
export type GuardLog = (tag: string, msg: string) => void;
...
// eslint-disable-next-line -- 原始 JSON 动态形态，保持与 JS 版相同的宽容读取
type JsonValue = any;

function createGuard(opts: GuardOpts) {
  const {
  ...
  const home = (): string => getHome() || path.join(os.homedir(), '.dsh');
  const profileDir = (): string => path.join(home(), 'profiles', getProfile());
  const guardDir = (): string => path.join(home(), 'guard');
  const rollbacksDir = (): string => path.join(home(), 'rollbacks', getProfile());
  const stateFile = (): string => path.join(guardDir(), 'state.json');
  const incidentsDir = (): string => path.join(guardDir(), 'incidents');

  function readJson<T = JsonValue>(file: string, fallback: T): T {
    try {
  ...
  function snapshot(reason: unknown): SnapshotMeta | null {
    try {
      ...
      log('guard', '创建快照失败: ' + (err instanceof Error ? err.message : String(err)));
      return null;
    }
  }
  ...
  function pruneSnapshots(): void {
    try {
      ...
      for (let i = MAX_SNAPSHOTS; i < list.length; i += 1) {
        fs.rmSync(path.join(rollbacksDir(), (list[i] as SnapshotMeta).id), { recursive: true, force: true, maxRetries: 2 });
      }
    } catch {
      /* 清理失败不影响主流程 */
    }
  ...
  function restore(id: unknown): RestoreResult {
    try {
      ...
      snapshot('pre-restore:' + String(id));
  ...
  function state(): GuardState {
    return readJson<GuardState>(stateFile(), {});
  }

  function markGood(id: unknown): void {
    try {
      const s = state();
      s.lastGood = (id as string) || null;
  ...
  function healthCheck(): { at: string; profile: string; findings: GuardFinding[] } {
  ...
  // 供宿主周期性轻量检查（不打扰用户，只返回是否有异动）。
  function junctionFindings(): GuardFinding[] {
  ...
  function shadowFindings(dir: string): GuardFinding[] {
    const out: GuardFinding[] = [];
    try {
      const fallbackDir = path.join(home(), 'profiles', 'node_modules');
  ...
  function patchRowIds(patch: unknown): string[] {
    const ids: string[] = [];
  ...
  function patchFindings(dir: string): GuardFinding[] {
    const out: GuardFinding[] = [];
    try {
  ...
  function trojanFindings(dir: string): GuardFinding[] {
    const out: GuardFinding[] = [];
    try {
      const builtinManifest = readJson<{ names?: string[] }>(path.join(dir, '.dsh-builtin-plugins.json'), { names: [] });
      const builtin = new Set(builtinManifest.names || []);
      const modulesDir = path.join(dir, 'node_modules');
      ...
      const walk = (d: string, depth: number): void => {
        ...
        for (const e of entries) {
          ...
          if (e.isDirectory()) {
            ...
            if (pkg && builtin.has(pkg.name as string)) continue; // 内置分发包不扫
            walk(p, depth + 1);
  ...
  // ── 修复执行器（只动插件/配置层）────────────────────────────────────
  function repair(findings?: unknown): { applied: string[] } {
    const applied: string[] = [];
    const list: Array<{ code: string }> = Array.isArray(findings)
    ...
    if (list.some((f) => f.code === 'SHADOW_COPY' || f.code === 'SHADOW_LINK')) {
      try {
        // 懒加载保持原语义：模块缺失时吞错记日志（编译产物同目录可解析）。
        const { healProfileModuleShadowing } = require('./profile-module-heal') as typeof import('./profile-module-heal');
        const removed = healProfileModuleShadowing(home(), getProfile(), (m) => log('guard', m));
        if (removed.length) applied.push('清理模块遮蔽: ' + removed.join(', '));
      } catch (err) {
        log('guard', '清理模块遮蔽失败: ' + (err instanceof Error ? err.message : String(err)));
      }
    }

    if (list.some((f) => f.code === 'PATCH_DUP_ID')) {
      try {
        const { removeBundledRowDuplicates } = require('./patch-row-heal') as typeof import('./patch-row-heal');
        const file = path.join(dir, 'cordis.patch.yml');
        ...
        try {
          bundled =
            (readJson<any>(path.join(dir, 'package.json'), {}) as any)?.dsh?.profile?.bundles || [];
        } catch {
          bundled = [];
        }
        ...
        if (removed.length) {
          patch = deduped;
          applied.push('移除与 bundle 重复的 patch 行: ' + removed.map(String).join(', '));
        }
        if (removed.length) fs.writeFileSync(file, patch);
      } catch (err) {
        log('guard', '修复 patch 行失败: ' + (err instanceof Error ? err.message : String(err)));
      }
    }
    ...
    try {
      ...
      for (const { full, rel } of fallbackPackages(fallbackDir)) {
        ...
        try {
          ...
          log('guard', `恢复 junction ${full} 失败: ` + (err instanceof Error ? err.message : String(err)));
        }
      }
      if (repaired.length) {
      ...
      log('guard', 'junction 归属修复失败: ' + (err instanceof Error ? err.message : String(err)));
    }
    return { repaired, unknown };
  }

  // ── 事故报告（plugin-guard 的 incident）──────────────────────────────
  function reportIncident(title: unknown, detail: unknown): { ok: boolean; file?: string; error?: string } {
    try {
  ...
  function readIncident(id: unknown): IncidentResult {
    try {
      if (!/^[\w.-]+\.md$/.test(String(id || ''))) return { ok: false, error: 'bad id' };
      const file = path.join(incidentsDir(), String(id));
  ...
  function resolveIncident(id: unknown): { ok: boolean; error?: string } {
    try {
      if (!/^[\w.-]+\.md$/.test(String(id || ''))) return { ok: false, error: 'bad id' };
      const file = path.join(incidentsDir(), String(id));
      if (!fs.existsSync(file)) return { ok: false, error: 'not found' };
  ...
  async function guardedBoot(
    ...
    opts: { preRetry?: (errText: string) => Promise<{ applied?: string[] } | boolean | null> | { applied?: string[] } | boolean | null } = {},
  ): Promise<string> {
    ...
    try {
      ...
      if (opts.preRetry) {
        try {
          const r = await opts.preRetry(String((firstErr instanceof Error && firstErr.message) || firstErr));
          if (r && Array.isArray((r as { applied?: string[] }).applied) && (r as { applied: string[] }).applied.length)
            preApplied = (r as { applied: string[] }).applied;
          else if (r) preApplied = ['配置级修复钩子已应用'];
        } catch (err) {
          log('guard', 'preRetry 钩子失败: ' + String((err instanceof Error && err.message) || err));
        }
      }
      ...
      if (fixable.length || preApplied.length) {
        ...
        if (all.length) {
          ...
          try {
            ...
            reportIncident(
              'boot-recovered',
              '首次启动失败，自动修复后恢复。\n修复项：\n- ' + all.join('\n- ') + '\n\n原始错误：\n' + String((firstErr instanceof Error && firstErr.message) || firstErr),
            );
            return url;
          } catch (secondErr) {
    ...
    if (good && (!bootSnap || good.id !== bootSnap.id)) {
      ...
      if (res.ok) {
        ...
        try {
          ...
          reportIncident('rollback-failed', '回滚到快照 ' + good.id + ' 后仍无法启动。\n\n最终错误：\n' + String((finalErr instanceof Error && finalErr.message) || finalErr));
          throw finalErr;
        }
      }
    }
    reportIncident('boot-failed', '启动失败且无可回滚快照。\n\n错误：\n' + String((err instanceof Error && err.message) || err) + (describeFailure ? '\n\n' + describeFailure() : ''));
    throw err;
  }

  // 回滚后的拉起也要留「最后良好」标记 —— 交给调用方包一层。
  let rollbackLift: (() => Promise<string>) | null = null;
  function setRollbackLift(fn: () => Promise<string>): void {
    rollbackLift = fn;
  }
  ...
  function attributeBootFailure(errText: unknown): { name: string; kind: 'patchRow' | 'bundle' | 'dependency'; rowId: string | null } | null {
    try {
      ...
      const manifest = readJson<any>(path.join(dir, 'package.json'), {});
      const bundles =
        manifest.dsh && manifest.dsh.profile && Array.isArray(manifest.dsh.profile.bundles) ? manifest.dsh.profile.bundles : [];
      const depKeys: string[] = Object.keys(manifest.dependencies || {});
